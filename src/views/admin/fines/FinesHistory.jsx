...
export default function AdminFinesHistory() {
  ...
  const [pageNumber, setPageNumber] = useState(0); // 0‑based, converted to 1‑based for API
  ...
  const handlePayFine = async (fineId) => {
    try {
      await fineApi.markFineAsPaid(fineId); // assuming this endpoint exists
      // Update local state
      setFines((prev) =>
        prev.map((f) =>
          f.id === fineId
  ...
  // Row helpers
  const startItem = fines[0]?.totalCount === 0 ? 0 : pageNumber * PAGE_SIZE + 1;
  const endItem = Math.min((pageNumber + 1) * PAGE_SIZE, fines[0]?.totalCount || 0);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 max-w-4xl">
            View all fines (paid and unpaid). Mark unpaid fines as paid once settled.
          </p>
        </div>
